...
const TableConfigItems = (props) => {
   ...
   const checkboxToggle = (product, config = {}) => {
      let ConfiguredItems = product.ConfiguredItems.map(configItem => configItem.Id === config.Id ? {
         ...config,
      ...
      let modified = cartConfigured.map(mapItem => mapItem.Id === updatedProduct.Id ? updatedProduct : mapItem);
