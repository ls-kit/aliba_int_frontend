...
export const numberWithCommas = (numericDtaa) => {
   if (numericDtaa) {
      ...
      return floatNum.toString().split('.')[0].length > 3 ? floatNum.toString().substring(0, floatNum.toString().split('.')[0].length - 3).replace(/\B(?=(\d{2})+(?!\d))/g, ",") + "," + floatNum.toString().substring(floatNum.toString().split('.')[0].length - 3) : floatNum.toString();
   }
   return "0.00";
};
      ...
      if (!_.isEmpty(Promotions) && _.isArray(Promotions)) {
         Price = Promotions.map(promotion => {
            ...
            if (_.isArray(promoConfig)) {
               const findPromoConfig = promotion.ConfiguredItems.find(configFilter => configFilter.Id === configuredId);
...
export const CartCheckedProductSubTotal = (Product) => {
   ...
   if (!_.isEmpty(productTotal)) {
      if (productTotal.totalPrice) {
         let otherCost = Number(Product.DeliveryCost) + (Number(Product.totalWeight) * Number(Product.ShippingRate));
         Subtotal = Number(productTotal.totalPrice) + Number(otherCost);
      }
   }
   return Subtotal;
...
export const cartProductQuantityUpdate = (qty, cartConfigured, product_id, existsConfigId, ShippingCharges) => {
